...
def generate_json_data():
    ...
    try:
        # send a post request with a timeout so your server doesn't hang forever
        r = requests.post(
        ...
        # Check if Ollama returned an error object instead of the answer
        if "error" in response_data:
            raise RuntimeError(f"Ollama Error: {response_data['error']}")
            
        # Return the text response from the AI safely
        return response_data["response"]

    except requests.exceptions.ConnectionError:
        raise RuntimeError("Could not connect to Ollama. Make sure the desktop app is active in your taskbar!")
    except KeyError:
        raise RuntimeError(f"Unexpected JSON structure returned from Ollama. Raw payload: {r.text}")